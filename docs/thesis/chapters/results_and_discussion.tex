...
\begin{itemize}
  ...
  \item \textbf{Dynamic Routing:} The app router under \code{frontend/src/app} manages path transitions based on roles. Access to restricted areas (e.g., \code{/admin/*} or \code{/supervisor/*}) triggers client-side role guards that inspect the active user's permissions.
...
\begin{itemize}
  \item \textbf{Page Speed:} Using Google PageSpeed Insights and Webpage Grader, the Next.js static asset bundling and Redis cached lesson routes show rapid load times. Main page loads reach a Time to Interactive (TTI) of 1.8 seconds.
...
\begin{itemize}
  \item \textbf{Cross-Browser Rendering:} Functionality is tested on Chromium (Chrome/Edge), WebKit (Safari), and Gecko (Firefox). The UI displays consistently across these rendering engines, with consistent KaTeX math notation alignment.
