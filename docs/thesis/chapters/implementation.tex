...
\subsubsection{Detailed API Endpoint Specification}

A comprehensive specification of the core platform endpoints is detailed in Table \ref{tab:api-spec}, defining the REST verbs, request payloads, success payloads, and RBAC visibility.

\begin{longtable}{L{0.32\textwidth}L{0.60\textwidth}}
\caption{Detailed REST API Specification}\label{tab:api-spec}\\
\toprule
\textbf{Method / Route} & \textbf{Functionality / Summary} \\
\midrule
\endfirsthead
\toprule
\textbf{Method / Route} & \textbf{Functionality / Summary} \\
\midrule
\endhead
\multicolumn{2}{c}{\textbf{Authentication and Profile Management}} \\
\midrule
\code{POST} \newline \code{/auth/register} & Create student or supervisor credentials \\
\code{POST} \newline \code{/auth/login} & Validate credentials and retrieve session JWT \\
\code{POST} \newline \code{/auth/refresh} & Re-issue active token on session refresh \\
\code{POST} \newline \code{/auth/upgrade-role} & Elevate user credentials context to supervisor \\
\code{GET} \newline \code{/auth/activity} & Retrieve activity logs and study duration counters \\
\midrule
\multicolumn{2}{c}{\textbf{Lessons and Hierarchy Management}} \\
\midrule
\code{GET} \newline \code{/lessons} & Retrieve paginated curriculum lessons \\
\code{POST} \newline \code{/lessons} & Create new lesson outline (Teacher/Admin only) \\
\code{GET} \newline \code{/lessons/subjects} & Retrieve authorized course subjects \\
\code{POST} \newline \code{/lessons/subjects} & Create new course subject definition \\
\code{GET} \newline \code{/lessons/practice-available} & List lessons flagged for dynamic practice \\
\code{GET} \newline \code{/lessons/mastery/all} & Fetch mastery tracking logs of the user \\
\midrule
\multicolumn{2}{c}{\textbf{Question Generation and Testing}} \\
\midrule
\code{GET} \newline \code{/tests/grade-tests} & List eligibility status for grade milestone tests \\
\code{POST} \newline \code{/tests/submit-answer} & Submit mathematical formula answer to check \\
\code{GET} \newline \code{/tests/templates} & Retrieve list of question templates \\
\code{POST} \newline \code{/tests/templates} & Create new question template configurations \\
\code{GET} \newline \code{/tests/my-practice-history} & Retrieve list of practice attempts \\
\midrule
\multicolumn{2}{c}{\textbf{Gamification and Social Matchmaking}} \\
\midrule
\code{POST} \newline \code{/achievements/check} & Evaluate and award unlocked achievements \\
\code{GET} \newline \code{/games/available} & Retrieve list of multiplayer challenges \\
\code{POST} \newline \code{/games/attempts} & Record score results from math challenge play \\
\midrule
\multicolumn{2}{c}{\textbf{Billing and Unit Administration}} \\
\midrule
\code{POST} \newline \code{/subscription/toggle-auto-renew} & Toggle Stripe auto-renew setting \\
\code{POST} \newline \code{/supervisor/buy-slots} & Buy sub-student seat spaces \\
\code{GET} \newline \code{/admin/roles} & Retrieve RBAC roles registry list \\
\code{GET} \newline \code{/admin/subject-access-requests} & List pending classified subject access requests \\
\bottomrule
\end{longtable}
...
A key operational challenge resolved in the deployment configuration involves build-time database constraints. During the Docker compilation phase on Render, the \code{DATABASE\_URL} environment variable is not injected (as it is scoped strictly to the application's runtime phase). The compiler initially failed when executing \code{npx\allowbreak prisma\allowbreak generate} because the local ORM configuration (\code{prisma.config.ts}) tried to resolve this variable dynamically using Prisma's \code{env()} configuration helper, which enforces lookup checks during build time. This constraint was resolved by refactoring the configuration file to reference the standard Node.js \code{process.env.DATABASE\_URL} lookup directly. This adjustment allows the client generation script to execute successfully during image compilation, decoupling the database connectivity verification from the build phase.
\end{sloppypar}
