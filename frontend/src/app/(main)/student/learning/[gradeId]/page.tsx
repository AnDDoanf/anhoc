...
import { Trophy, ArrowLeft, Layers, Target } from "lucide-react";
...
export default function GradePathPage() {
  ...
  // Find the current grade group
  const gradeGroup = lessonStructure.find((g) => g.grade === gradeId);

  if (!gradeGroup) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6">
        <h1 className="text-4xl font-black text-sol-text">{commonT("error.gradeNotFound")}</h1>
        <Link href="/student/learning" className="text-sol-accent hover:underline">{commonT("backToHub")}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12 space-y-16">
      ...
      <div className="relative pb-32">
        {gradeGroup.lessons.map((lesson, idx) => {
