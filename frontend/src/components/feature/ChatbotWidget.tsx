...
export default function ChatbotWidget() {
  ...
  const [activeSection, setActiveSection] = useState<"chat" | "thought" | "context">("chat");
  ...
  const chatbotApiUrl = process.env.NEXT_PUBLIC_CHATBOT_API_URL || "http://localhost:5002";
  ...
  useEffect(() => {
    ...
    try {
      ...
      setMessages(parsed);
      const lastTutorMsg = [...parsed].reverse().find((message: Message) => message.sender === "tutor");
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    localStorage.setItem(`chatbot_history_${user.id}`, JSON.stringify(updatedHistory));
    setLoading(true);
    ...
    try {
      ...
      const handleStreamEvent = (rawEvent: string) => {
        ...
        if (event.type === "error") {
          throw new Error(event.detail || t("errors.serverError"));
        }
      };
      ...
      setMessages(streamedHistory);
      localStorage.setItem(`chatbot_history_${user.id}`, JSON.stringify(streamedHistory));
      ...
      localStorage.setItem(`chatbot_history_${user.id}`, JSON.stringify(restored));
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed bottom-6 right-6 z-[999] select-none font-sans">
      ...
      {isOpen && (
        <div className="flex h-[36rem] w-[24rem] sm:w-[28rem] flex-col rounded-[2.5rem] border border-sol-border/30 bg-sol-surface shadow-2xl transition-all duration-300 overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-3 border-b border-sol-border/10 text-center text-xs font-black uppercase bg-sol-surface/50">
            {[
              { id: "chat", label: t("tabs.chat"), icon: <MessageSquare size={13} /> },
              { id: "thought", label: t("tabs.thought"), icon: <Brain size={13} /> },
              { id: "context", label: t("tabs.context"), icon: <Info size={13} /> }
            ].map((section) => (
              <button
                key={section.id}
                onClick={() => setActiveSection(section.id as "chat" | "thought" | "context")}
          ...
          <div className="flex-1 overflow-y-auto p-5 bg-sol-bg/20">
            {activeSection === "chat" && (
              <div className="space-y-4 h-full flex flex-col justify-between overflow-hidden">
                <div className="flex items-center gap-2 overflow-x-auto pb-2 mb-1 border-b border-sol-border/10 scrollbar-none shrink-0 select-none">
                  <span className="text-[10px] font-black text-sol-muted uppercase tracking-wider shrink-0 pl-1">
                    {t("engineLabel")}
                  </span>
                    ...
                    {providerOptions.map((option) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-xl text-[10px] font-bold uppercase transition-all cursor-pointer border flex items-center gap-1 shrink-0
                            ${
                              isSelected
                                ? "bg-sol-accent text-sol-bg border-sol-accent shadow-sm"
                                : "bg-sol-surface/50 border-sol-border/20 text-sol-muted hover:text-sol-text hover:bg-sol-bg/50"
                            }
                          `}
                        >
                          {option.name}
                          <span
                            className={`text-[8px] px-1 py-0.2 rounded-md font-extrabold uppercase tracking-tight
                              ${
                                isSelected
            ...
            {activeSection === "thought" && (
              <div className="space-y-4">
                <div className="flex items-center gap-2 text-sol-accent border-b border-sol-border/10 pb-3">
                  <Brain size={18} />
                  <h5 className="text-sm font-black uppercase tracking-wider">{t("thoughtTitle")}</h5>
                </div>
                {lastThought ? (
                  <div className="rounded-[1.5rem] border border-sol-border/20 bg-sol-surface p-5 text-sm font-medium text-sol-text whitespace-pre-wrap leading-relaxed shadow-sm font-mono text-[12px]">
                    {lastThought}
                  </div>
                ) : (
                  <div className="text-center py-20 text-sol-muted font-bold text-xs italic">
                    {t("thoughtEmpty")}
                  </div>
                )}
              </div>
            ...
            {activeSection === "context" && (
              <div className="space-y-6">
                <div className="flex items-center gap-2 text-sol-accent border-b border-sol-border/10 pb-3">
                  <Info size={18} />
                  <h5 className="text-sm font-black uppercase tracking-wider">{t("contextTitle")}</h5>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="rounded-2xl border border-sol-border/20 bg-sol-surface p-3 text-center shadow-sm">
                    ...
                    <p className="text-base font-black text-sol-text mt-0.5">
                      {lastContext?.total_xp ?? (user as any).student_stats?.total_xp ?? 0}
                    </p>
                  </div>
                  ...
                  <div className="rounded-2xl border border-sol-border/20 bg-sol-surface p-3 text-center shadow-sm">
                    ...
                    <p className="text-base font-black text-sol-text mt-0.5">
                      {lastContext?.level ?? (user as any).student_stats?.level ?? 1}
                    </p>
                  </div>
